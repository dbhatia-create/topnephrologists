...
export async function sendApplyToBff(
  ...
  // Cities actually receiving featured placement: this site models "featured"
  // as an opt-out (featuredPlacement on by default, excludedFeatured lists the
  // "city|state" keys the applicant opted out of), unlike the reference site's
  // opt-in featuredLocations array — so we derive the included set here.
  const featuredCities = data.featuredPlacement
    ? data.locations.filter((l) => !excludedFeatured.includes(`${l.city}|${l.state}`))
    : [];

  // Listing tier the applicant bought: "featured" if any city is actually
  // receiving featured placement, otherwise "paid" (a basic listing always
  // costs at least one city fee).
  ...
  const payload = {
    ...
    // This site's schema has no executives/key-staff field — default to empty.
    key_staff: [] as string[],
    cities: data.locations.map((l) => `${l.city}, ${l.state}`),
    featured_cities: featuredCities.map((l) => `${l.city}, ${l.state}`),
